...
<!--button type="button" class="btn btn-primary" data-toggle="modal" data-target="#dailytaskModal"> Launch project_task modal</button-->

<!-- project_task Modal Starts -->

<div class="modal fade" id="dailytaskModal" tabindex="-1" role="dialog" aria-labelledby="dailytaskModalLabel" aria-hidden="true">
  <div class="modal-dialog modal-lg" role="document">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="dailytaskModalLabel">Project Task</h5>
      ...
      <div class="modal-body">
        <form id="project_taskformid">
          ...
          <div class="form-group col-md-6">
              ...
              <select class="form-control" name="selectEmployeeFromSelect2DropdownId" id="selectEmployeeFromSelect2DropdownId">
            ...
            <div class="form-group col-md-3">
                    ...
                    <input type="date" id="dailytask_date" class="form-control" placeholder="Select Date" name="dailytask_date" required="required" value="{{ date('Y-m-d') }}" autocomplete="off" min="{{ date('Y-m-d', strtotime(date('Y-m-d') . ' -9999 days')) }}" max="{{ date('Y-m-d') }}">
                  </div>
          </div>
          <div class="row">
...
$('body').on('click', '#project_taskformsaveid', function () {
		...
		var hidadmin_id = $('#selectEmployeeFromSelect2DropdownId').val();
		var dailytask_date = $('#dailytask_date').val();
        ...
        if(hidselected_projectcategory_id=='' || id=='' || project_task_description=='' || hidselected_project_id=='' || project_dailytask_hours=='' || dailytask_date=='' || hidadmin_id=='' || project_dailytask_minutes=='' || dailytask_is_active!=1){
		...
		$("#dailytaskModal").modal('hide');
     	...
     	$.ajax({
            url: "{{ route('add-new-dailytask-byadmin') }}",
            data: {
              _token:'{{ csrf_token() }}',
              hid: id, 
			  ...
			  admin_id: hidadmin_id,
			  task_description: project_task_description,
			  is_active: dailytask_is_active,
			...
			error: function(jqXHR, textStatus, errorThrown) {
                showSweetAlert('Something Went Wrong!','An error occurred, Please Refresh Page & Try again.', 0); 
            },
        });
  });


  function showAddNewDailyTaskManuallyModal(idx=0){
                    ...
                    delay: 250,
                     data: function (params) {
                        return {
							...
							_token:'{{ csrf_token() }}',
							'includeid': 0,
							'admin_id': $('#selectEmployeeFromSelect2DropdownId').val(), 
                        };
                    },
...
var selectEmployeeFromSelect2DropdownId = $("#selectEmployeeFromSelect2DropdownId").select2({
    		placeholder: "Select An Admin/Employee ",
    		width:"100%",
                ajax: {
					url: "{{ route('getAdminInDropdown') }}",
                    type: "post",
                    dataType: 'json',
                    delay: 250,
                     data: function (params) {
